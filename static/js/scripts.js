...
function obtenerObjetoGol(){//TODO ESTO ES PROVISIONAL, EN REALIDAD SE DEBEN PERMITIR SELECCIONAR JUGADORES EN EL SELECTOR,ESTO SOLO ES POSIBLE DESDE LA VISTA HTML
    let tumama = "Jugador 1";
...
function botonTarjeta(localia, tarjeta, agregarQuitarBool) {
  //true = estadistica local; false= estadistica visitante
  ...
  let llamarContador = "";
    ...
    if( tarjeta == "Amarilla" && localia == true){
        llamarContador = "contadorAmarillasLocal";
    }else if( tarjeta == "Amarilla" && localia == false){
        llamarContador = "contadorAmarillasVisitante";
    }else if( tarjeta == "Roja" && localia == true){
        llamarContador = "contadorRojasLocal";
    }else if( tarjeta == "Roja" && localia == false){
        llamarContador = "contadorRojasVisitante";
    }else{
        console.log("Error en el llamado a contador");
    }
    ...
    if (agregarQuitarBool == true) {
        if(contador.value < 11){
            if (tarjeta == "Roja" && contador.value >= 4){
                window.alert("No pueden haber más de 4 rojas por equipo");
            } else {
            contador.value++;            
            }
        } else {
            window.alert("No pueden haber más de 11 tarjetas amarillas por equipo"); //SE PONEN POR DEFECTO AMARILLAS PUESTO QUE NO SE PUEDEN TENER MÁS DE 4 TARJETAS ROJAS
        }       
    } else {
        contador.value > 0 ?  contador.value--: window.alert("No se pueden quitar más tarjetas");
    }
